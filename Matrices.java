...
public class Matrices {
    ...
    public static class Vector {
        ...
        public Vector add(Vector other) {
            ...
            Vector resust = new Vector(this.items.length);

            for (int i=0; i<this.items.length; i++){
                resust.items[i] = this.items[i] + other.items[i];
            }
            return resust;
        }
        ...
        public int dotProduct(Vector other) {
            ...
            int multiply=0;
            for (int i=0;i<this.items.length; i++){
                multiply += Math.abs(this.items[i])*Math.abs(other.items[i]);
            }
            return multiply;
        }
    ...
    public static class Matrix {
        ...
        public Matrix add(Matrix other) {
            ...
            for (int i=0; i<result.rows.length; i++){
                for (int j=0; j<result.rows[0].length; j++){
...
//                }
//            }
            determinantOfMatrix = this.rows[0][0]*this.rows[1][1]-this.rows[0][1]*this.rows[1][0];
    ...
    public static void main(String[] args) {
        ...
        Vector zeroVector = new Vector(4);
